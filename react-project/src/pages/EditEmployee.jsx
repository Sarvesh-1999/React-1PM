...
const EditEmployee = () => {
  ...
  useEffect(()=>{
    async function getEditEmployee() {
        try {
          let resp = await axios.get(`http://localhost:5000/employees/${params.id}`)
          console.log(resp);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/employees/${params.id}`, employee)
      navigate("/all")
  ...
  return (
    <article className="h-screen w-full bg-zinc-100 px-20">
      <h1 className="py-5 font-bold text-xl">Edit Employee</h1>
